...
- `show_intermediate_results=true` возвращает `dense/lex/fusion/ce`, если эти данные были сформированы во fresh pipeline или сохранены в cache payload.
- `show_intermediate_results` не влияет на cache read: чтение кеша контролируется только `search_use_cache`.
