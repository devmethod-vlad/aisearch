...
Контекст: `devmethod-vlad/aisearch`, реализация фильтрации по полям `role`, `product` и `component`.
...
role = "Врач;Медсестра"
product = "ЭМИАС;ЛИС"
component = "Назначения;Расписания"
```
...
- исходные raw-поля `role`, `product` и `component` сохраняются как есть;
...
TOKEN_FILTERS_RAW_FIELDS=role,product,component
...
| --- | --- | --- |
| `TOKEN_FILTERS_RAW_FIELDS` | `role,product,component` | Список raw-полей, для которых нужно создавать token-поля |
...
component + _tokens = component_tokens
```
...
Это важно: загрузка данных и поиск должны одинаково понимать, что такое `role_tokens`, `product_tokens` и `component_tokens`.
...
Это гарантирует, что при первичной загрузке записи получают `role_tokens`, `product_tokens` и `component_tokens` до записи в Milvus и OpenSearch.
...
Значит, `role_tokens`, `product_tokens` и `component_tokens` попадают и в Milvus, и в OpenSearch при инкрементальных обновлениях.
...
```text
ARRAY_CONTAINS(component_tokens, "назначения") AND ARRAY_CONTAINS(product_tokens, "эмиас") AND ARRAY_CONTAINS(role_tokens, "врач")
```
...
3. Добавить поле в env:
   ...
   ```env
   TOKEN_FILTERS_RAW_FIELDS=role,product,component,department
   ```
...
Проверяет, что `prepare_dataframe` добавляет `role_tokens`, `product_tokens` и `component_tokens`.
...
Если экспорт должен быть полным техническим snapshot с token-полями, нужно добавить `role_tokens`, `product_tokens` и `component_tokens` в порядок экспортируемых колонок или отдельно документировать, что token-поля при повторной загрузке будут пересозданы из raw-полей.


## 23. Важно после изменения схем

После добавления нового token-поля `component_tokens` необходимо пересоздать OpenSearch index и Milvus collection (или выполнить полный reindex), чтобы схема и данные стали консистентными.


## API-контракт (актуально с 2026-05-07)
Во внешнем API используется имя `array_filters` внутри `filters`, а внутренние
переменные runtime-пайплайна по-прежнему могут называться `token_filters`.
Это один и тот же набор условий в разных слоях системы.
