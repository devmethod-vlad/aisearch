...
from app.domain.exceptions import QueueMaxSizeException
...
class HybridSearchService(IHybridSearchService):
    ...
    async def enqueue_search(
        ...
        Внешний API использует `array_filters`, а внутри orchestrator они
        нормализуются в token-представление для OpenSearch/Milvus.
        """
        ...
        try:
            ...
            raise QueueMaxSizeException("Search queue overflow, try later") from e
