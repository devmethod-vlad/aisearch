...
#             if not container:
#                 raise self.retry(
#                     exc=Exception("Dishka container not initialized"), countdown=5
#                 )
...
#             )
#             logger.error(
#                 f"Ошибка в задаче поиска ({type(error)}): {traceback.format_exc()}"
#             )
#             await redis_watcher.lrem(pkey, 1, ticket_id)
#             return {"status": "failed"}
...
#                             lex = await _os_candidates(
#                                 os_adapter, query, settings.lex_top_k
#                             )
#                         elif switches.use_bm25:
#                             lex = await _bm25_candidates(
#                                 bm25, query, settings.lex_top_k
#                             )
...
#                         for m, s in zip(merged, ce_scores, strict=True):
#                             m["score_ce"] = float(s)
#                     _results = _score_and_slice(
#                         merged, top_k, use_ce=switches.use_reranker
#                     )
...
#     bm25: IBM25Adapter, query: str, k: int
# ) -> list[dict[str, tp.Any]]:
...
#     return "\n".join(
#         [m.get("question", ""), m.get("analysis", ""), m.get("answer", "")]
#     )
