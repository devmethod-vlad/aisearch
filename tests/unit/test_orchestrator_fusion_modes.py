...
def test_weighted_ce_blend_computes_norms_and_weighted_sum() -> None:
    ...
    o = _build_orchestrator()
    items = [
        {"ext_id": "a", "score_fusion": 2.0, "score_ce": 0.8, "score_ce_raw": 10.0},
        {"ext_id": "b", "score_fusion": 1.0, "score_ce": 0.2, "score_ce_raw": 0.0},
    ]
    results = o._score_and_slice(
        items,
        2,
...
def test_ce_blend_with_zero_ce_weight_disables_ce_contrib() -> None:
    ...
    o = _build_orchestrator()
    items = [
        {"ext_id": "a", "score_fusion": 2.0, "score_ce": 0.0},
        {"ext_id": "b", "score_fusion": 1.0, "score_ce": 1.0},
    ]
    results = o._score_and_slice(
        items,
        2,
...
def test_rrf_ce_blend_weights_normalized_and_ce_source_variants() -> None:
    ...
    o = _build_orchestrator()
    base = [
        {"ext_id": "a", "score_fusion": 2.0, "score_ce": 0.1, "score_ce_raw": 10.0},
        {"ext_id": "b", "score_fusion": 1.0, "score_ce": 0.9, "score_ce_raw": 20.0},
    ]
    processed = o._score_and_slice(
    ...
    assert raw[0]["score_ce_norm"] == 1.0
    assert raw[1]["score_ce_norm"] == 0.0
...
def test_should_run_cross_encoder_modes() -> None:
    ...
    assert not o._should_run_cross_encoder(
        items=items, final_rank_mode="fusion_only", final_w_ce=1.0
    )
    assert o._should_run_cross_encoder(
        items=items, final_rank_mode="ce_final", final_w_ce=0.0
    )
    assert o._should_run_cross_encoder(
        items=items, final_rank_mode="ce_blend", final_w_ce=0.1
    )
    assert not o._should_run_cross_encoder(
        items=items, final_rank_mode="ce_blend", final_w_ce=0.0
    )
    assert o._should_run_cross_encoder(
        items=items, final_rank_mode="legacy_weighted", final_w_ce=0.1
    )
    assert not o._should_run_cross_encoder(
        items=items, final_rank_mode="legacy_weighted", final_w_ce=0.0
    )
...
def test_build_hybrid_version_contains_new_final_settings_only() -> None:
    ...
    assert ":final_w_ce=0.2" in v3
    assert ":final_w_fusion=0.8" in v3

    assert ":final_fusion_norm=minmax" in v4
    assert ":final_ce_score=raw" in v4

    assert ":final_rank_mode=fusion_only" in v1
    assert ":final_rank_mode=ce_final" in v2
...
def test_apply_short_settings_overrides_final_and_switches() -> None:
    ...
    updated_settings, updated_switches = o._apply_short_settings(
        settings_local=settings_local, switches_local=switches_local
    )
